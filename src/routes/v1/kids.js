...
kids_router.route('/').get(async(c) => {
    
    let kidslist = await c.db.select().from(kids).where(and(eq(plans.type, 'pase'), or(eq(kids.status, 'active'), eq(kids.status, 'finish'))))
...
kids_router.route('/').post(zValidator('json', kidsPostValidator), async (c) => {
    ...
    await c.db.insert(kids_plans).values({kidId: kid[0].id, planId: data.planId, date: moment.tz(moment(), 'America/Santiago')})
    c.status(201)
    c.header('HX-Trigger', 'refreshKids')
    return c.text('Niño ingresado!')
})

kids_router.route('/:id/timer').put(async (c) => {
    
    let id = await c.req.param('id')

    let result = await c.db.update(kids).set(await c.req.json()).where(eq(kids.id, id))
